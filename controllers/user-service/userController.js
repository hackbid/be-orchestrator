"use strict";
const userApi = process.env.PORT || "https://users-api.hackbid.com";
  ...
  static async register(req, res, next) {
    try {
      ...
      if (data) {
        ...
        const params = {
          Bucket: "hackbid",
          Key: Math.random() * 1000 + `${username}_profile`,
          Body: buffer,
        };
        ...
        uploadParallel.done().then(async (image) => {
          try {
            const imageProfile = image.Location;
            const { data: dataImage } = await axios.post(
              userApi + `/image/${data.id}`,
              {
                imageProfile,
              }
            );
          } catch (err) {
            console.log(err, "<<< ini response");
            throw err;
          }
        });
      }
