...
module.exports = class UserController {
    ...
    static async addBalance(req, res, next) {
        try {
            ...
            const { data: userData } = await axios.get(userApi + `/users/${userId}`);
    ...
    static async getWithdraw(req, res, next) {
        try {
            ...
            let temp = dataWithdraw.map(async (e) => {
                const { data: dataUser } = await axios.get(userApi + `/users/${e.UserId}`);
    ...
    static async approvalWD(req, res, next) {
        try {
            const { id } = req.params;
            const { data: resApproval } = await axios.patch(userApi + `/approve/${id}`);
    ...
    static async rejectWD(req, res, next) {
        const { id } = req.params;
        try {
            const { data: resReject } = await axios.patch(userApi + `/reportwd/${id}`);
            const { data: userData } = await axios.get(userApi + `/users/${id}`);
