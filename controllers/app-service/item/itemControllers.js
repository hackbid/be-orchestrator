...
module.exports = class ItemController {
    ...
    static async findItemToday(req, res, next) {
        try {
            ...
            const temp = itemToday.map(async (e) => {
                const { data: imagesData } = await axios.get(mongoAPI + `/itemImages/${e.imageMongoId}`);
    ...
    static async postBid(req, res, next) {
        ...
        const { id } = req.params;
        try {
            const { data: dataBid } = await axios.post(mongoAPI + `/itemHistory/${id}/bid`, {
                UserId,
                username,
                bidValue,
            });
    ...
    static async itemHistory(req, res, next) {
        try {
            const { id } = req.params;
            const { data: historyItems } = await axios.get(mongoAPI + `/itemHistory/${id}`);
